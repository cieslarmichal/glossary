...
const std::string sentenceWithoutTags{
    sentenceMark +
...
TEST_F(
    GlossaryHtmlParserTest,
    givenHtmlContentWithDefinitionSequenceAndExample_shouldReturnParsedContent)
{
    const std::string testHtmlContent{taggedDefinition + taggedSentence +
                                      taggedExample};
    const std::vector<std::string> testParsedContent{
        definitionWithoutTags, sentenceWithoutTags, exampleWithoutTags};
       ...
       givenHtmlContentWithoutImportantLines_shouldReturnEmptyVector)
{
    ...
    const auto actualParsedHtmlContent =
        htmlParser.parse(contentWithoutImportantLines);
