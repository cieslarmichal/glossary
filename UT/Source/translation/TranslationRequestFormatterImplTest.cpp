...
{
    const auto formattedRequest = formatter.getFormattedRequest(emptyText, SourceLanguage::Polish, TargetLanguage::English);
...
{
    const auto actualFormattedRequest = formatter.getFormattedRequest(singleWordText, SourceLanguage::Polish, TargetLanguage::English);

    ASSERT_EQ(actualFormattedRequest, requestWithSingleWord);
}

TEST_F(TranslationRequestFormatterImplTest, givenThreeWordsText_shouldReturnRequestWithTheseWordsSplitByPlusSignInTextField)
{
    const auto actualFormattedRequest = formatter.getFormattedRequest(multipleWordsText, SourceLanguage::Polish, TargetLanguage::English);
