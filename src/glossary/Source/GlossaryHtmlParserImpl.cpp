...
const std::string htmlDefinitionTag{
    R"(<span class="dtText"><strong class="mw_t_bc">)"};
...
const std::string htmlExampleTag3{
    "<span class=\"ex-sent first-child t no-aq sents\">"};
...
std::map<std::string, std::string> tagPrefixMapping =
    boost::assign::map_list_of(htmlExampleTag1, examplePrefix)(
        htmlExampleTag2, examplePrefix)(htmlExampleTag3, examplePrefix)(
        htmlSentenceTag1, sentencePrefix)(htmlSentenceTag2, sentencePrefix);
...
std::vector<std::string> GlossaryHtmlParserImpl::selectGlossaryLines(
...
bool containsAnyTagWithEndTag(const std::string& content)
{
    const auto containsDefinitionTag =
        containsTagWithEndTag(content, htmlDefinitionTag);
    const auto containsExample1Tag =
        containsTagWithEndTag(content, htmlExampleTag1);
    const auto containsExample2Tag =
        containsTagWithEndTag(content, htmlExampleTag2);
    const auto containsExample3Tag =
        containsTagWithEndTag(content, htmlExampleTag3);
    const auto containsSentenceTag1 =
        containsTagWithEndTag(content, htmlSentenceTag1);
    const auto containsSentenceTag2 =
        containsTagWithEndTag(content, htmlSentenceTag2);
    return containsDefinitionTag || containsExample1Tag ||
           containsExample2Tag || containsExample3Tag || containsSentenceTag1 ||
           containsSentenceTag2;
}
...
bool containsTagWithEndTag(const std::string& content, const std::string& tag)
{
    ...
    {
        const auto nearestEndTag =
            findNearestEndTagPosition(content, tagPosition);
...
TagWithPosition findNearestEndTagPosition(const std::string& content,
                                          size_t startTagPosition)
{
    TagWithPosition prolongation1{
        htmlFindProlongationTag1,
        content.find(htmlFindProlongationTag1, startTagPosition + 1)};
    TagWithPosition prolongation2{
        htmlFindProlongationTag2,
        content.find(htmlFindProlongationTag2, startTagPosition + 1)};
    TagWithPosition end{htmlEndTag,
                        content.find(htmlEndTag, startTagPosition + 1)};
    while ((prolongation1.position < end.position ||
            prolongation2.position < end.position) &&
