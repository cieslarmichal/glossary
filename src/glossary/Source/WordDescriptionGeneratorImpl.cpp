...
WordDescriptionGeneratorImpl::WordDescriptionGeneratorImpl(
    std::unique_ptr<WordDescriptionService> service)
    : wordDescriptionService{std::move(service)}
{
}

WordsDescriptions WordDescriptionGeneratorImpl::generateWordsDescriptions(
    const EnglishWords& englishWords) const
{
    ...
    for (const auto& englishWord : englishWords)
    {
        ...
        std::cout << "Downloading wordsDescriptions "
                  << ++wordDescriptionCounter << "/" << englishWords.size()
                  << "\n";
    }
    return wordsDescriptions;
}

WordDescription WordDescriptionGeneratorImpl::generateWordDescription(
